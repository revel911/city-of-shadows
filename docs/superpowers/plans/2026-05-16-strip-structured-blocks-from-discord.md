...
Add this block immediately after the step-3 loop:
...
```js
  ...
  // and unterminated containers have been stripped to end-of-string. Any
  // remaining standalone <TAG> or </TAG> for a tag in ORPHAN_TAGS is by
  // definition orphaned (no counterpart). Strip them as bare literals.
  // No body inspection — these are text fragments with no payload.
  for (const tag of ORPHAN_TAGS) {
    // Strip closing tags first so any prior content stays intact.
    const closeRe = new RegExp(`<\\/${tag}>`, 'g');
    if (closeRe.test(working)) {
      working = working.replace(closeRe, '');
      leakDetected = true;
    }
    const openRe = new RegExp(`<${tag}>`, 'g');
    if (openRe.test(working)) {
      working = working.replace(openRe, '');
      leakDetected = true;
    }
  }
```
